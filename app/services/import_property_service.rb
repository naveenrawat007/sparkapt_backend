        ...
        if data[0].present? && !Property.find_by(name: data[0].to_s).present?
          ...
          @params[:properties][0].slice(1,5).each do |property_type|
            if property_type.include?("1")
              ...
              property_type_detail = type.type_details.create(size: data[12].to_f, price: data[11].to_s.gsub(/[^0-9A-Za-z]/, '').to_f, available: data[13].to_s)
            elsif property_type.include?("2")
              type = Type.find_by(type_code: 'bedroom_2')
                property_type_detail = type.type_details.create(size: data[15].to_f, price: data[14].to_s.gsub(/[^0-9A-Za-z]/, '').to_f, available: data[16].to_s)
            elsif property_type.include?("3")
              type = Type.find_by(type_code: 'bedroom_3')
              property_type_detail = type.type_details.create(size: data[18].to_f, price: data[17].to_s.gsub(/[^0-9A-Za-z]/, '').to_f, available: data[19].to_s)
            elsif property_type.include?("Studio")
              type = Type.find_by(type_code: 'studio')
              property_type_detail = type.type_details.create(size: data[9].to_f, price: data[8].to_s.gsub(/[^0-9A-Za-z]/, '').to_f, available: data[10].to_s)
            end
